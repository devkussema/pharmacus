...
@push('scripts')
    ...
    <script>
        ...
        async function carregarDashboard() {
            try {
                ...
                const stats = {
                    ...
                    alertasCriticos: 34,
                    variacaoProdutos: 12,
                    variacaoMovimentacoes: 8,
                    percentualNormal: 85,
                    percentualCriticos: 6,
                    statusNormal: 491,
                    statusMinimo: 49,
                    statusCritico: 34,
                    statusExpirando: 28
                ...
                const criticos = [{
                    ...
                    },
                    {
                        ...
                        nivel: 'warning',
                        nome: 'Dipirona 1g',
                        mensagem: 'Nível mínimo - 15 unidades'
                    }
                ];

                atualizarEstatisticas(stats);
                atualizarAtividades(atividades);
                atualizarProdutosCriticos(criticos);
            } catch (error) {
        ...
        function atualizarEstatisticas(stats) {
            document.getElementById('totalProdutos').textContent = stats.totalProdutos || 0;
            document.getElementById('movimentacoes').textContent = stats.movimentacoes || 0;
            document.getElementById('estoqueNormal').textContent = stats.estoqueNormal || 0;
            document.getElementById('alertasCriticos').textContent = stats.alertasCriticos || 0;

            document.getElementById('variacao-produtos').textContent = (stats.variacaoProdutos || 0) + '%';
            document.getElementById('variacao-movimentacoes').textContent = (stats.variacaoMovimentacoes || 0) + '%';
            document.getElementById('percentual-normal').textContent = (stats.percentualNormal || 0) + '%';
            document.getElementById('percentual-criticos').textContent = (stats.percentualCriticos || 0) + '%';

            document.getElementById('statusNormal').textContent = stats.statusNormal || 0;
            document.getElementById('statusMinimo').textContent = stats.statusMinimo || 0;
            document.getElementById('statusCritico').textContent = stats.statusCritico || 0;
            document.getElementById('statusExpirando').textContent = stats.statusExpirando || 0;
            document.getElementById('criticalCount').textContent = stats.alertasCriticos || 0;
        }
        ...
        <div class="activity-item">
            <div class="activity-icon ${ativ.tipo}">
                <i class="fa-solid ${getIconeAtividade(ativ.tipo)}"></i>
            </div>
            <div class="activity-content">
                <div class="activity-title">${ativ.titulo}</div>
                <div class="activity-meta">${ativ.usuario} • ${ativ.tempo}</div>
            </div>
            ${ativ.badge ? `<span class="activity-badge">${ativ.badge}</span>` : ''}
        </div>
    `).join('');
        }

        function atualizarProdutosCriticos(criticos) {
            const container = document.getElementById('criticalProducts');

            if (!criticos || criticos.length === 0) {
                container.innerHTML = '<p class="text-muted text-center">Nenhum produto crítico</p>';
                return;
            }

            container.innerHTML = criticos.map(produto => `
        <div class="alert-item ${produto.nivel}">
            <div class="alert-icon">
                <i class="fa-solid ${produto.nivel === 'critical' ? 'fa-times-circle' : 'fa-exclamation-triangle'}"></i>
            </div>
            <div class="alert-content">
                <div class="alert-title">${produto.nome}</div>
                <div class="alert-message">${produto.mensagem}</div>
            </div>
            <button class="alert-action" onclick="verDetalhes(${produto.id})">
    ...
    `).join('');
        ...
        function verDetalhes(id) {
            window.location.href = `/diretor/estoque/${id}`;
        }
    </script>
@endpush
